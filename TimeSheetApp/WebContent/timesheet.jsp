...
<!-- Loads JQuery from Google CDN -->
	...
	// Retrieves changeWeek Form POST from previous and next week user navigation and instantiates the Projects object with the correct week in the past
	//session.setAttribute("weeksInPast", (String) request.getParameter("changeWeek") == null ? Integer.toString(0) : Integer.toString(Integer.parseInt((String) session.getAttribute("weeksInPast")) + Integer.parseInt((String) request.getParameter("changeWeek"))));
	session.setAttribute("weeksInPast", request.getParameter("changeWeek") == null || Integer.parseInt((String) session.getAttribute("weeksInPast")) == 0 && Integer.parseInt((String) request.getParameter("changeWeek")) == -1 ? Integer.toString(0) : Integer.toString(Integer.parseInt((String) session.getAttribute("weeksInPast")) + Integer.parseInt((String) request.getParameter("changeWeek"))));
	Projects user1 = new Projects((String) session.getAttribute("Employee_ID"), Integer.parseInt((String) session.getAttribute("weeksInPast")));

	//Projects user1 = new Projects((String) session.getAttribute("Employee_ID"), Integer.parseInt((String) session.getAttribute("weeksInPast")) < 0 ? 0 : Integer.parseInt((String) session.getAttribute("weeksInPast")));
	%>   
        ...
        <thead>
		  ...
		  <tr>
			...
			<%for (int day = 0; day < 7; day++) {%>
				<th id="<%=day%>" class="date"><%=user1.getDateLabels(day)%></th>
			<%}%>
			<!--
...
<%--         <tbody>
        ...
        <tfoot>
	    	<td>
				<select id="project" onclick="addProject(this)">
...
<form id="previousWeekForm" method="post" action="timesheet.jsp">
	<input id="previousWeek" name="changeWeek" type="hidden" value="1"></input>		      
	<button class="col-sm-offset-1 col-sm-2 glyphicon glyphicon-fast-backward btn btn-info btn-lg" type="submit"></button>
</form>
<form id="nextWeekForm" method="post" action="timesheet.jsp">
	<input id="nextWeek" name="changeWeek" type="hidden" value="-1"></input>
	<button class="col-sm-offset-1 col-sm-2 glyphicon glyphicon-fast-forward btn btn-info btn-lg" type="submit" <%=Integer.parseInt((String) session.getAttribute("weeksInPast")) == 0 ? "disabled" : ""%>></button>
</form>
